module Garnix.TestHelpers.HetznerMock
  ( HetznerState (HetznerState),
...
testHetznerInterface :: HetznerInterface
testHetznerInterface =
  HetznerInterface
    { _hetznerInterfaceProvisionServer = \serverId _location _type -> do
        now <- liftIO getCurrentTime
        hetznerServerId' <- liftIO $ HetznerServerId <$> randomIO
        ...
        liftIO $ modifyMVar_ (_getHetznerState hetznerState) (pure . Map.insert hetznerServerId' (containerId, Left info, mvar))
        return info,
      _hetznerInterfaceUpdateMetadata = \_repoInfo deploymentType build serverId hetznerId -> do
        now <- liftIO getCurrentTime
        let upd x = case x of
                ...
                let s =
                      ServerInfo
                        { _serverInfoId = serverId,
                          _serverInfoHetznerServerId = hetznerId,
                          ...
                          _serverInfoBuildPersistenceName = Nothing, -- TODO
                          _serverInfoTier = def,
                          _serverInfoIsPrimary = False
                        }
                 in Just (t, Right s, mv)
        liftIO $ modifyMVar_ (_getHetznerState hetznerState) $ \m -> pure $ Map.alter upd hetznerId m,
      _hetznerInterfaceDeleteServer = \hetznerId -> liftIO $ do
        let upd = \case
              Nothing -> pure Nothing
              Just (thread, _, _) -> deleteContainer thread $> Nothing
        modifyMVar_ (_getHetznerState hetznerState) (Map.alterF upd hetznerId),
      _hetznerInterfaceGetServerStatus = \hetzerId -> do
        m <- liftIO $ readMVar (_getHetznerState hetznerState)
        case Map.lookup hetzerId m of
          Nothing -> throw $ ProvisioningError "No such server"
          Just (_, _, mvar) -> liftIO $ readMVar mvar
    }

newtype HetznerState = HetznerState
  { _getHetznerState ::
      MVar
        ( Map.Map
            HetznerServerId
...
{-# NOINLINE hetznerState #-}
hetznerState :: HetznerState
hetznerState = unsafePerformIO $ HetznerState <$> newMVar mempty
...
uncachedGetVmScript = do
  withSystemTempDirectory "garnix-vm" $ \dir -> do
    writeFile (dir </> "flake.nix") hetznerMockNixosConfig
    ...
    StdoutUntrimmed result <-
      run
        ...
        & addArgs
          [ "build",
            ".#nixosConfigurations.hetzner-mock.config.system.build.vm",
            "--json",
            "--no-link" :: String
          ]
        & nixConfDefaults
    ...
    let !bin = case result ^? nth 0 . key "outputs" . key "out" . _String of
          Nothing ->
            error
              $ "Could not get 'outputs' of hetzner-mock VM in "
              <> show result
          Just o -> cs o </> "bin/run-hetzner-mock-vm"
    pure bin

hetznerMockNixosConfig :: String
hetznerMockNixosConfig =
  [i|
      {
        ...
        outputs = { self, nixpkgs} :
          ...
          in
          {
            nixosConfigurations.hetzner-mock = nixpkgs.lib.nixosSystem {
              system = "x86_64-linux";
              modules = [
                ...
                ({ ... }: {
                  ...
                  networking.hostName = "hetzner-mock";
