{
  ...
  outputs =
    ...
    }:
    {
        ...
        modules = [
          garnix-ci.nixosModules.garnix-guest
          {
            garnix.server = {
              ...
              # Optional SSH access to the guest's `garnix` user (login-closed
              # and password-auth-off by default). `exposeSSH` only opens a
              # public DNAT port; authorize a login too via the fields below
              # (or reach it over tailscale / ProxyJump without exposeSSH).
              # exposeSSH = true;
      ...
      # Same page, locked behind Authentik login via garnix-authentik. Set
      # `garnix.server.deployment` above (mirroring `hello`) to deploy it, and
      # fill in the Authentik OIDC details. See docs/authentik-cookbook.md.
